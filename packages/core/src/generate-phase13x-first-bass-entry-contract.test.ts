...
import {
  ...
  PHASE_13X_FIRST_BASS_ENTRY_REVIEW_SEEDS,
...
test("Phase 13X first-bass evidence is separate from post-exposition bass-entry evidence", () => {
  const metrics = collectPhase13XFirstBassEntryMetrics(PHASE_13X_FIRST_BASS_ENTRY_REVIEW_SEEDS);

  assert.equal(metrics.seedCount, PHASE_13X_FIRST_BASS_ENTRY_REVIEW_SEEDS.length);
