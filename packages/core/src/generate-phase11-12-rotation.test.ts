...
test("generateScore applies phase-11 and phase-12 phrase-unit planning across rotation seeds", () => {
  const metrics = collectPhase1112PlanningMetrics(PHASE_11_12_ROTATION_SEEDS);
  ...
  assert.ok(metrics.variantLeapRecoveryMisses <= metrics.baselineLeapRecoveryMisses + 24);
