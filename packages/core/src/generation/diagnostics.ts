...
import { analyzeMeterConsistency } from "./meter-consistency-review.js";
import { analyzePhraseRepetitionReviewSummary } from "./phrase-repetition-review-summary.js";
...
function summarizeMetricalHarmony(
  ...
  for (const { tick, active } of verticalities) {
    ...
    const plan = sectionPlanForTick(sectionPlans, tick);
    const anchor = nearestHarmonicAnchor(tick, sectionPlans);
    if (anchor === undefined || active.size === 0 || plan === undefined) {
      continue;
    }
    ...
    const beatStrength = beatStrengthAtTick(tick, plan.meterContext);
    const isStrongBeat = beatStrength === "strong";
