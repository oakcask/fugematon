...
export function annotateEpisodeMotivicDerivations(
  notes: NoteEvent[],
  ...
  for (const note of notes) {
    ...
    note.motivicDerivation = deriveMotivicPlan(note, plan, notes, subjectEntries);
  }
}
...
function deriveMotivicPlan(
  ...
  const preparesNextEntry =
    nextEntryAfterPlan(plan, subjectEntries) !== undefined &&
    note.startTick >= plan.startTick + Math.max(plan.meterContext.beatTicks, plan.durationTicks / 2);
  return {
    sourceMotive: sourceMotiveForNote(note, plan, notes),
...
function sourceMotiveForNote(
  ...
  if (hasPriorEpisodeFigure(notes, note, plan)) {
...
function nextEntryAfterPlan(plan: HarmonicPlan, subjectEntries: readonly PlannedEntry[]): PlannedEntry | undefined {
  const planEndTick = plan.startTick + plan.durationTicks;
  return subjectEntries
    .filter((entry) => entry.startTick >= planEndTick)
    .sort((left, right) => left.startTick - right.startTick)[0];
}

function hasPriorEpisodeFigure(notes: readonly NoteEvent[], note: NoteEvent, plan: HarmonicPlan): boolean {
  return notes.some(
    (candidate) =>
      candidate.role === "free-counterpoint" &&
      candidate.startTick < plan.startTick &&
      note.startTick - candidate.startTick <= plan.meterContext.measureTicks * 4 &&
      Math.abs(candidate.pitch - note.pitch) <= 7 &&
