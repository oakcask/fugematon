...
test("generateScore reports contour motion diagnostics", () => {
  ...
  assert.equal(output.diagnostics.selectedCandidateEvaluations[0]!.featureVersion, 9);
  ...
  assert.equal(output.diagnostics.selectedCandidateEvaluations[0]!.evaluationModelVersion, 17);
  assert.ok(
