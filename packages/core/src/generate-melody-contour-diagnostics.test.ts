...
test("generateScore reports contour motion diagnostics", () => {
  ...
  assert.equal(output.diagnostics.selectedCandidateEvaluations[0]!.featureVersion, 10);
  ...
  assert.equal(output.diagnostics.selectedCandidateEvaluations[0]!.evaluationModelVersion, 19);
  assert.ok(
