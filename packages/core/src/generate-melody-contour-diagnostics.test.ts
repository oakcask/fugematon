...
test("generateScore reports contour motion diagnostics", () => {
  ...
  assert.equal(output.diagnostics.selectedCandidateEvaluations[0]!.featureVersion, 7);
  ...
  assert.equal(output.diagnostics.selectedCandidateEvaluations[0]!.evaluationModelVersion, 13);
  assert.ok(
