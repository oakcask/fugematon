...
reviewTest("generateScore completes phrase-repetition adoption across review seeds batch A1b", () => {
  assertPhraseRepetitionReviewBatch(PHRASE_REPETITION_REVIEW_BATCH_A.slice(3, 6), 3, -3);
});
...
function assertPhraseRepetitionReviewBatch(
  ...
  assert.ok(metrics.variantUnsupportedThinningRuns <= metrics.baselineUnsupportedThinningRuns);
}
