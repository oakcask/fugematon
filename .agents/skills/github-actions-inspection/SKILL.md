...
3. When log or artifact output needs repeated searching, fetch or save the GitHub data once into a disposable cache file, then run local searches against that file.
...
```sh
gh run view RUN_ID --log > .cache/github/run-log.txt
rg -i "failure|error|timeout" .cache/github/run-log.txt
```
