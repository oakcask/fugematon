...
- Do not make human manual listening a blocker for an implementation target. Complete agent-side score review, record any listening gap as non-blocking, and continue when no unresolved score blocker remains.
- Block implementation on a high-confidence musical failure visible in the score even when no current metric detects it; localize the evidence and follow the `music-theory-review` score-blocker rules.
