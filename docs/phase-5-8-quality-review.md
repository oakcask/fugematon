...
* 自動 diagnostics が通っても、手動聴取で退屈、機械的、声部が独立して聞こえないと判断した seed は Phase 6 へ進めない。
...
Phase 6 は、Phase 5.8-5.12 の美しさ gate が代表 seed と境界 seed で通った後に開始する。UI 操作やパラメータで退屈さを隠すのではなく、生成器そのものが美しいフーガ的構造を作れることを先に確認する。
