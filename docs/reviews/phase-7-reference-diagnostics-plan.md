...
この audit から、既存 pool の selection model だけで Phase 7 completion criteria を満たす経路は現時点でない。PR2 oracle の selection/generator 切り分けは有効だが、Phase 7 完了には section-local planner が新しい候補を作り、reference-relative diagnostics と manual pairwise listening の両方で現行より勝つ必要がある。manual listening が欠けている限り、diagnostics の小改善だけで Phase 7 を complete とは扱わない。
