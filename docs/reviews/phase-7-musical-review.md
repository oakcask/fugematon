...
最初の section planner 変更では、非 modal seed に限って 7 continuation cycle ごとに隣接 pattern へ 1 cycle だけ切り替え、同じ 4-section pattern の固定反復を少し崩した。22 seed の Phase 6/7 gate は pass し、4-section continuation pattern の最大反復は 7 以下、unique pattern count 合計は 112 になった。代表 seed では `fugue-smoke` unique 6 / most 7、`long-arc` unique 7 / most 7、`lyrical-line` unique 5 / most 7、`contrary-answer` unique 6 / most 7。modal seed は pattern を固定したままにし、`modal-answer`、`modal-dorian`、`modal-cadence`、`dense-modal`、`angular-answer` の modal identity と contour gate を守った。
...
9. Form and long-run interest: episode/codetta/stretto preparation、段階的 thinning、register/density variation を section planner の責務にし、manual listening gate と pairwise preference を Phase 8 前の blocker として埋める。非 modal continuation pattern rotation で long-run repetition は少し弱まったが、selected section solo texture risk は最大 8 のまま残るため、次は second support voice を単純に足すのではなく、voice-pair unison、leap recovery、modal identity、outer-voice contour を同時に守る section-local texture planner として扱う。
