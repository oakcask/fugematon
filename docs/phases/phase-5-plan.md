...
Phase 4 完了後の複数 seed レビューでは、構造 diagnostics は安定した一方で、全 seed の note 数、entry 数、状態遷移が同型になり、対旋律、episode、cadence、stretto の音楽的説得力が不足していることが分かった。詳細は `../reviews/phase-4-quality-review.md` を参照する。
...
リポジトリ上の Phase 5 自動完了条件は満たしている。ただし、Phase 5 review bundle の再レビューでは、音楽的美しさを Phase 6 の前提にするには追加の品質フェーズが必要だと判断した。詳細は `../reviews/phase-5-quality-review.md` を参照する。
...
当初は Phase 6 の履歴、巻き戻し、操作パラメータへ進む前に、Phase 5.6 以降の品質フェーズを追加した。Phase 5.11 で固定 seed、rotation seed、adversarial seed の gate と margin follow-up を review bundle に固定できたため、Phase 5 はここで完了とする。残る旋律線、entry 支持和声、フレーズ整形、評価説明力は Phase 6 以降の音楽生成コア改善として扱い、操作機能はさらに後ろへ送る。
...
1. hard constraint gate
   * 声域違反、声部交差、主題同一性違反、応答計画違反、key metadata mismatch、説明できない完全同音ユニゾン、明確な未解決不協和などを扱う。
   * ここは手書きルールを正とし、学習済み重みで許可しない。
2. rule-based soft score
   * leap recovery、melodic stagnation、同音連打、unison pressure、same-pitch overlap pressure、same-motion pressure、rhythmic independence、entry density、episode direction、stretto clarity などを扱う。
...
7. Phase 5.11 で Phase 5 を完了し、rotation seed と margin gate を後続フェーズの観察基盤として固定する。
8. Phase 6 として、旋律線、跳躍回収、フレーズ境界、装飾配置を整える。
9. Phase 7 として、手調整の `EvaluationWeights`、pairwise preference、必要に応じた learned weights の A/B review を扱う。
10. Phase 8 の操作パラメータは、美しさ gate と評価器の feature/weight が説明可能になってから、次の状態遷移以降に反映する。
...
Phase 5.7 後の複数 seed review では、hard constraints は安定したが、声部独立、旋律線、リズム対位法、装飾の意味付け、美しさ diagnostics の説明力がまだ弱いと分かった。詳細は `../reviews/phase-5-8-quality-review.md` を参照する。
...
* 自動 diagnostics が通っても、聴取で退屈、機械的、声部が独立して聞こえないと判断した seed は操作機能フェーズへ進めない。
...
* 自動 diagnostics が通っても、manual judgement が `pass` でない seed を操作機能フェーズへ進めない運用にできる。
* pairwise preference を後続の evaluation weight 改善へ渡す形式がある。

### Phase 5.9: review seed 全体の美しさ gate

Phase 5.8 後の再レビューでは、hard constraints は全 14 seed で 0 のままだが、全 seed で texture independence warning と leap recovery miss が残った。詳細は `../reviews/phase-5-9-quality-review.md` を参照する。
...
* manual listening judgement が `pass` でない代表 seed または境界 seed は操作機能フェーズ前の blocker として扱う。
...
* `listening-review.json` は、代表 seed と境界 seed の manual listening judgement が `pass` でない場合に操作機能フェーズ前 blocker を残す。
...
* review bundle には Phase 5.9 gate 結果が残り、手動聴取 gate が `pass` でない代表 seed と境界 seed は操作機能フェーズ前 blocker になる。
* Phase 5.9 は品質 gate の固定であり、声部独立とリズム対位法の実質改善は Phase 5.10、rotation seed と margin gate は Phase 5.11、旋律線とフレーズ整形は Phase 6、評価内訳と preference の説明可能化は Phase 7 で扱う。
...
Phase 5.10 は、Phase 5.9 の review seed gate を保ったまま、声部独立と entry 周辺の濁りを操作機能フェーズ前の自動 gate として固定した。
...
Phase 5.10 後の複数 seed review では、固定 review seed は自動 gate を通過したが、追加 rotation seed で rhythmic independence、entry support instability、modal counter-subject identity が崩れた。詳細は `../reviews/phase-5-10-quality-review.md` を参照する。

このため、操作パラメータへ進む前に、Phase 5.11 として rotation seed と margin gate を追加する。固定 seed の pass は必要条件に留め、閾値に張り付く seed と追加 seed の fragility を美しさの blocker として扱う。
...
* manual listening judgement が `pass` でない代表 seed と境界 seed は、引き続き操作機能フェーズ前 blocker として review bundle に残る。

#### Phase 5.11 後の再レビュー

Phase 5.11 後の複数 seed review では、固定 review seed、rotation seed、adversarial seed はすべて自動 gate を通過したが、20 seed に margin follow-up が残った。特に entry support instability の局所最大と連続発生、modal seed の counter-subject identity、shared rhythm overlap、leap recovery miss が閾値付近に張り付いている。また、22 seed すべてで 3 パート休止による solo texture が cadence や phrase boundary なしに現れ、episode 後半の continuity が 1 声 filler に寄る構造的な問題も確認した。詳細は `../reviews/phase-5-11-quality-review.md` を参照する。

Phase 5.11 は、品質問題をすべて解消するフェーズではなく、固定 seed への過適合と rotation seed の fragility を検出できる観察基盤を作るフェーズとして完了した。このため Phase 5 はここで一区切りとし、Phase 6 では旋律線、entry 支持和声、装飾、フレーズ境界、提示部から継続部への音楽的展開、3 パート休止へ入る説得力を候補生成と scoring の両方で改善する。Phase 7 では follow-up の理由を seed、section、voice、entry/cadence 周辺の単位で説明できるようにする。
...
* 提示部は 4 声の主題・応答 entry を固定間隔で置くだけで終えず、codetta、短い追唱、最後の応答後の呼吸を候補に入れる。
* exposition duration、最初の continuation 開始位置、entry 間隔を diagnostics に出し、全 seed で同じ提示部長に張り付く場合は follow-up にする。
* episode は主題先頭 4 音の fragment だけでなく、sequence、invertible counterpoint、free counterpoint が前の entry を受けて発展しているかを評価する。
* 3 パート休止による solo texture は、cadence、phrase boundary、応答的な受け渡し、または段階的な thinning によって説明できる場合だけ許容する。episode 後半を空いている 1 声だけで埋める continuity は、最低 texture density と休止理由を持つ phrase plan へ置き換える。
* solo run count、unsupported solo run count、abrupt texture drop count、solo voice imbalance を diagnostics に出し、`fugue-smoke` だけでなく 22 seed 全体で観察する。
* `fugue-smoke`、`bach-001`、`minor-entry`、`wide-key`、`long-arc`、`tight-stretto`、`dense-modal` は、提示部の長さと continuation 接続の境界 seed として扱う。
...
* entry 周辺の m2、M2、m7、M7 は severe seconds/sevenths として 4 度や三全音とは分けて診断する。4 パート texture では半音・全音衝突が和音全体を支配しやすいため、テンションノート、掛留、経過音、刺繍音、導音などの役割と解決先を説明できない場合は、軽い減点ではなく採用回避に近い強いコストとして扱う。
* `lyrical-line`、`circle-fifths`、`close-imitation`、`contrary-motion` は severe seconds/sevenths の追加境界 seed として扱う。
...
* texture density が下がる箇所は、なぜ 1 声だけが残り、他の 3 パートが休むのかを section、phrase、cadence の文脈で説明できるようにする。
...
* 操作パラメータの UI は Phase 6-7 の美しさ gate、声部独立、旋律線、評価内訳整備後、Phase 8 で扱う。
* 生成探索の Worker 化は Phase 9 で扱う。
