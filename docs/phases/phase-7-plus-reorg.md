...
そのうえで、操作機能より音楽美を優先するため、Phase 8/9 はいったん deferred operational lane に送り、先に Phase 10 quality foundation を実施した。Phase 10 は reference corpus、candidate pool oracle、A/B review、pairwise preference、section-local planner の品質基盤を揃えたため完了扱いにする。ただし Phase 10 後の譜面レビューで、無限再生へ進むには品質不足が明確になったため、現在の実装対象は Phase 8 ではなく Phase 11 の品質モデル再設計へ移す。
...
* manual listening は未実施のため、Phase 8/9 後の継続 quality lane として残す。`minor-entry` の leap recovery、`modal-cadence` の counter-subject identity retention、`bach-001` / `fugue-smoke` の unison/shared rhythm、`dense-modal` の bass/root support は次の listening または support formula refinement で確認する。

### Phase 8: 操作機能 MVP

Phase 8 は Phase 11 の品質 baseline 採用後に戻る deferred operational lane とする。Phase 7B 時点で開始可能な safety baseline と Phase 10/11 compatibility baseline を維持し、操作 UI が review signal を消さないようにする。
...
Phase 10 で品質基盤を先行したが、Phase 10 後の譜面レビューにより、Phase 8/9 の前に Phase 11 quality model rebuild を挟む。Phase 11 後に Phase 8/9 へ戻った後も、以下は継続 lane として残る。
...
Phase 11 の変更は破壊的な生成モデル変更を許容するが、hard constraints、determinism、schema compatibility、reference diagnostics summary、candidate-pool oracle shape は維持する。review signal の悪化は seed、section、metric、音楽的 tradeoff を記録したうえで採否を判断する。
...
1. `harmony-diagnostics` の strong beat / harmonic function mismatch を実測 diagnostic にする。
2. ScoreEvent から adjacent voice interval、register span、active voice count away from cadence、state grammar repetition、entry pattern family を集計する review summary を追加する。
3. HarmonicPlan と subject generation の接続を作り、strong beat structural note と weak beat non-chord tone を候補生成時に持たせる。
4. 代表 4 seed と full 22 seed で譜面レビューを再生成し、Phase 10 score review の blocker が下がったか確認する。
