...
Phase 10 は Phase 7B の hard constraint policy と oracle evidence を前提に、操作機能より先に実施した。詳細な完了 evidence は [Phase 10](phase-10.md) に置く。
...
Phase 11 は、Phase 8/9 の無限再生・操作機能へ進む前に、生成モデルそのものを破壊的変更も含めて作り直す品質フェーズである。詳細は [Phase 11](phase-11.md) に置く。
...
Phase 13 は、Phase 12 後の unresolved musical costs を単独 metric のしきい値ではなく、正規化された quality vector の統計的な外れ方として扱う。詳細は [Phase 13](phase-13.md) に置く。
...
Phase 8 は Phase 13 の quality vector review model を持った後に戻る deferred operational lane とする。Phase 7B 時点で開始可能な safety baseline と Phase 10/11/12 compatibility baseline は維持するが、操作 UI が similar phrase blocker や Phase 12 後の unison / repeated-note defects を隠す設計にならないようにする。
...
* 操作後の generated score も reference diagnostics、review signal、quality vector comparison を出せる。
...
Phase 10 で品質基盤を先行したが、Phase 10 後の譜面レビューにより Phase 11 quality model rebuild を挟んだ。Phase 11 後の 22 seed review でも similar phrase blocker が残ったため、Phase 8/9 の前に Phase 12 phrase/repetition quality rewrite を挟んだ。Phase 12 後の human feedback で残った欠陥は Phase 13 quality vector statistical review で review/adoption model 化する。Phase 13 後に Phase 8/9 へ戻った後も、以下は継続 lane として残る。
...
Phase 11/12 の変更は破壊的な生成モデル変更を許容したが、Phase 13 は first pass では selected output を変えない。将来の生成変更でも hard constraints、determinism、schema compatibility、reference diagnostics summary、candidate-pool oracle shape は維持する。review signal の悪化は seed、section、metric、音楽的 tradeoff を記録し、quality vector と local sentinel の外れ方を説明したうえで採否を判断する。
...
1. Phase 12 selected output を変えずに、duration-based voice-pair unison と soprano repeated-note diagnostics を追加する。
2. review bundle summary に `qualityVector` と `qualityProfileComparison` を追加する。
3. 22 seed Phase 12 baseline review で、median、p90、max、outside seed count、top contributing axes、local sentinel outlier を記録する。
4. `review-ab` の採否 summary を、blocker improvement、quality vector movement、local sentinel regression、manual listening gap の並びで出す。
5. pairwise listening evidence が蓄積してから、quality vector の小さく説明可能な subset を candidate scoring に使えるか検証する。
