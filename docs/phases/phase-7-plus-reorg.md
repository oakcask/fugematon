...
そのうえで、操作機能より音楽美を優先するため、Phase 8/9 はいったん deferred operational lane に送り、先に Phase 10 quality foundation を実施した。Phase 10 は reference corpus、candidate pool oracle、A/B review、pairwise preference、section-local planner の品質基盤を揃えたため完了扱いにする。現在の実装対象は Phase 8 に戻す。
...
* Phase 10 完了後の現在は Phase 8 に戻る。Phase 8 が開始可能であることは、継続 quality lane を止める意味ではなく、Phase 8 以降の変更が守るべき compatibility baseline として扱う。
...
Phase 8 は Phase 10 の品質基盤後に戻った current operational lane とする。Phase 7B 時点で開始可能な safety baseline と Phase 10 completion compatibility baseline を維持し、操作 UI が review signal を消さないようにする。
...
Phase 10 で品質基盤を先行したが、すべての quality work を一度に完了させる必要はない。Phase 8/9 へ戻った後も、以下は継続 lane として残る。
...
1. Phase 8 ring buffer replay と rewind の最小 data model を追加する。
2. parameter-change meta event を deterministic に記録し、次の状態遷移以降へだけ影響させる。
3. 操作前後の diagnostics、reference comparison、review signals、manual listening note を review bundle で比較できるようにする。
4. Phase 9 の Worker 化は Phase 8 の replay / parameter history が安定してから扱う。
