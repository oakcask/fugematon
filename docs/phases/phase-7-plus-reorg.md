...
Phase 10 で品質基盤を先行したが、Phase 10 後の譜面レビューにより Phase 11 quality model rebuild を挟んだ。Phase 11 後の 22 seed review でも similar phrase blocker が残ったため、Phase 8/9 の前に Phase 12 phrase/repetition quality rewrite を挟む。Phase 12 後に Phase 8/9 へ戻った後も、以下は継続 lane として残る。
...
1. subject stem、answer transform、fragment derivation reason、phrase function、section-state pattern の repetition-family diagnostics を追加する。
2. selected output を変えずに phrase-family generator candidates を作り、Phase 11 current に対する oracle upper bound を確認する。
3. `angular-answer`、`modal-dorian`、`modal-answer`、`modal-cadence`、`dense-modal` の focused A/B review で phrase family selection を検証する。
4. phrase-level harmonic rhythm と planned support counterpoint を追加し、unsupported thinning 改善と voice-leading tradeoff を同時に確認する。
