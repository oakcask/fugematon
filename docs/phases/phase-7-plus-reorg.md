...
* 操作後の generated score も reference diagnostics、review signal、quality vector comparison、performance profile metadata を出せる。
...
Phase 10 で品質基盤を先行したが、Phase 10 後の譜面レビューにより Phase 11 quality model rebuild を挟んだ。Phase 11 後の 22 seed review でも similar phrase blocker が残ったため、Phase 8/9 の前に Phase 12 phrase/repetition quality rewrite を挟んだ。Phase 12 後は Phase 12P で演奏プロファイル境界を組み込み、human feedback で残った欠陥は Phase 13 quality vector statistical review で review/adoption model 化する。Phase 13 後に Phase 8/9 へ戻った後も、以下は継続 lane として残る。
...
1. Phase 12 selected output を変えずに、`packages/performance`、default profiles、`ScoreEvent` から `PerformanceEvent` への変換を追加する。
2. MIDI export と WebAudio playback を共有 `PerformanceProfile` interpretation に通し、review artifact に profile id と version を残す。
3. Phase 12 selected output を変えずに、duration-based voice-pair unison と soprano repeated-note diagnostics を追加する。
4. review bundle summary に `qualityVector` と `qualityProfileComparison` を追加する。
5. 22 seed Phase 12 baseline review で、median、p90、max、outside seed count、top contributing axes、local sentinel outlier を記録する。
6. `review-ab` の採否 summary を、blocker improvement、quality vector movement、local sentinel regression、manual listening gap の並びで出す。
7. pairwise listening evidence が蓄積してから、quality vector の小さく説明可能な subset を candidate scoring に使えるか検証する。
