...
* [Constrained keyboard writing profiles](constrained-keyboard-writing-profiles.md): planned follow-up。ピアノ、チェンバロ、プログラマブルオルゴール向けに `WritingProfile` を追加し、音域、使用可能 pitch set、手内スパン、同時発音、反復速度を generation 側の制約として扱う。`PerformanceProfile` は再生解釈の境界として分離したままにする。
