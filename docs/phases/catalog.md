...
* [Continuous fugue boundary carry planning](continuous-fugue-boundary-carry-planning.md): planned current target。`continuous-fugue` が hidden boundary で exposition reset を避けても、直前の声部・和声 carry が弱いと新しい開始に聞こえる問題を generator / diagnostics で扱う。
