...
* [Endless program terminal coda planning](endless-program-terminal-coda-planning.md): planned current target。`endless-program` の終端を、最後の短い terminal sonority override ではなく、planner-visible な coda section / terminal section intent として生成する。休止声部の final-boundary 同時復帰、短すぎる終止、fallback closure と generated coda quality の混同を扱う。
