...
* [Endless program terminal cadence validation](endless-program-terminal-cadence-validation.md): planned current work。`endless-program` を UI で連続再生する前に、segment 末尾が楽曲的に終止できるかを score evidence で検証し、必要なら generator / diagnostics を修理する。
