...
* [Endless program terminal coda historical style repair](endless-program-terminal-coda-historical-style-repair.md): planned current target。`endless-program` の terminal coda が直前の曲調を無視した全声部ロングトーンに聞こえる問題を、歴史的な final subject / stretto / pedal / liquidation / texture compaction の archetype と diagnostics で扱う。
