...
* [Infinite playback MVP](phase-8.md): 現在の実装対象。旧 Phase 8。Score-led beauty handoff と Metrical generation repair の review-required tradeoff を見える状態に保ちながら、無限再生セッション、境界設計、内部状態 visualizer の MVP を扱う。
...
* 現在の実装対象を見る場合は [Infinite playback MVP](phase-8.md) を読む。直前の Metrical generation repair handoff baseline は [Metrical generation repair](metrical-generation-repair.md) と [../reviews/metrical-generation-repair-completion.md](../reviews/metrical-generation-repair-completion.md) を読む。
