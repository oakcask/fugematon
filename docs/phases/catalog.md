...
* [Constrained keyboard writing profiles](constrained-keyboard-writing-profiles.md): completed follow-up。ピアノ、チェンバロ、プログラマブルオルゴール向けに `WritingProfile` を追加し、音域、使用可能 pitch set、手内スパン、同時発音、反復速度を generation 側の制約として扱う。`PerformanceProfile` は再生解釈の境界として分離したままにする。
* [Initial subject rhetoric diversity repair](initial-subject-rhetoric-diversity-repair.md): 完了済み quality follow-up。主題が毎回似通って聞こえる問題を、固定 profile lookup ではなく opening gesture、climax、tail motion、rhythm、meter stress、modal color を持つ constrained subject-rhetoric generator と top-N diversity diagnostics で扱った。完了 evidence は [Initial subject rhetoric diversity completion review](../reviews/initial-subject-rhetoric-diversity-completion.md)。
* [Playback source realism feasibility](playback-source-realism-feasibility.md): 調査完了。SpessaSynth + SoundFont、VSCO WAV サンプラー、SFZ 直接利用、OSS / 音源ライセンス表示ページの実現可能性を整理し、次段階は notices infrastructure と SoundFont pilot を推奨する。
* [Endless program terminal stretta planner](endless-program-terminal-stretta-planner.md): planned follow-up。現行 `endless-program` coda が cadence-safe でも取ってつけた終止に聞こえる問題を、coda 専用 planner と terminal stretta 優先の終止設計として扱う。
* [Endless program coda quality repair](endless-program-coda-quality-repair.md): 完了済み current target。`endless-program` terminal coda の archetype vocabulary が `stretto-compaction` に寄り、`final-fragment-entry` と `pedal-entry-cadence` が review-visible に現れない問題を、archetype selection、terminal coda derivation preservation、`codaContinuity` schema version 2 で扱った。完了 evidence は [Endless Program Coda Quality Gap Review](../reviews/endless-program-coda-quality-gap-review.md)。
* [Endless program terminal coda historical style repair](endless-program-terminal-coda-historical-style-repair.md): 完了済み current target。`endless-program` の terminal coda が直前の曲調を無視した全声部ロングトーンに聞こえる問題を、歴史的な final subject / stretto / pedal / liquidation / texture compaction の archetype と `codaContinuity` diagnostics で扱った。完了 evidence は [Endless Program Terminal Coda Review](../reviews/endless-program-terminal-coda-review.md)。
* [Continuous fugue boundary carry planning](continuous-fugue-boundary-carry-planning.md): 完了済み current target。`continuous-fugue` が hidden boundary で exposition reset を避けても、直前の声部・和声 carry が弱いと新しい開始に聞こえる問題を generator / diagnostics で扱った。完了 evidence は [Continuous Fugue Boundary Carry Review](../reviews/continuous-fugue-boundary-carry-review.md)。
...
* 主題が毎回似通う問題の修正案は [Initial subject rhetoric diversity repair](initial-subject-rhetoric-diversity-repair.md) を読む。Phase 13R の subject-diversity repair は旧三 family collapse の履歴として扱い、現在の fixed-profile / top-N rhetoric concentration 問題とは分けて読む。
