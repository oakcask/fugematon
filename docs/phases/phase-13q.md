...
Phase 13Q は、Phase 13 の quality vector review model を使って、Phase 13R と Phase 8/9 の前に生成結果そのものを改善する品質フェーズである。

Status: automatic adoption complete; manual listening follow-up remains open. Phase 13Q is inserted after Phase 13 and before Phase 13R. It extends candidate diversity beyond the Phase 12 phrase-unit baseline, preserves the focused Phase 12 phrase/repetition gains, and reduces the main Phase 13 review-required signals in the 22 seed automatic bundle: pitch-class unison duration, duration-based lockstep, and unresolved entry severe interval duration. The generated listening and pairwise preference templates are still `not-reviewed`, so this is diagnostics-backed adoption rather than human-listening signoff. Phase 8 remains deferred because Phase 13R must still align the normal generation path with the adopted baseline and repair the remaining long-run phrase convergence.
...
Manual MIDI listening remains open under `organ-default` and `strict-counterpoint`. Phase 8 may not proceed directly from Phase 13Q. The next phase is [Phase 13R](phase-13r.md), which must make the adopted model the actual default path and address the remaining phrase-convergence evidence in [Phase 13R convergence review](../reviews/phase-13r-convergence-review.md). Human preference claims must not treat Phase 13Q as manually listened until the generated listening templates are filled.
