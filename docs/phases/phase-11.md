...
1. Phase 11 review summary を追加する。ScoreEvent から adjacent voice interval、register span、active voice count away from cadence、state grammar repetition、entry pattern family、strong beat harmonic support を集計する。
2. `harmony-diagnostics` の未実装扱いになっている strong beat / harmonic function mismatch を実測 diagnostic にする。
3. Candidate pool oracle に Phase 11 blocker family を追加し、selection-only upper bound と generator-needed rate を review bundle に出す。
4. HarmonicPlan と subject generation の接続を作り、strong beat structural note と weak beat non-chord tone を候補生成時に持たせる。
5. Section grammar と register planner の候補を増やし、oracle が `generator-or-section-planner` と分類した blocker に対して候補 pool 自体を改善する。
6. Optional lane として offline learned selection weight を試す。採用判断ではなく、oracle-best と manual pairwise preference を説明できる feature を見つけるための実験にする。
7. 代表 4 seed と full 22 seed で譜面レビューを再生成し、Phase 10 score review の blocker が下がったか確認する。
