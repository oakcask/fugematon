...
6. 次に実装: multi-section phrase planner を追加し、2-4 section の phrase unit、cadence plan、density arc、entry plan、local key distance、subject family rotation を同時に候補化する。最初の採用条件は、`fugue-smoke` の最頻 4-section pattern count を baseline 以下へ戻し、`bach-001`、`modal-cadence`、`dense-modal` の section grammar improvement を失わないことである。
7. 次に実装: phrase unit に functional thinning annotation と support voice formula を持たせる。unsupported solo run と abrupt texture drop は、cadence extension、entry preparation、echo、pedal、suspension preparation、cadential preparation のどれにも結びつかない場合は generator-needed として扱う。
