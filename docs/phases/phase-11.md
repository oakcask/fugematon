...
Status: complete. Phase 8/9 は削除しないが、Phase 11 後の 22 seed review で similar phrase blocker が残ったため、Phase 12 phrase/repetition quality rewrite を挟んだ。Phase 11 の generator-quality baseline は採用済みで、manual listening は継続 quality lane として残す。現在の Phase 8/9 前 follow-up は Phase 12P と Phase 13 に移っている。
...
8. 部分完了: multi-section phrase-unit planner を selected output に入れ、2-4 section 単位で state sequence、cadence kind、density arc、local key distance を同時に選ぶ。22 seed では hard constraints と Phase 7B readiness を維持し、section grammar selected risk は 1602 から 486、unique 4-section continuation pattern 合計は 112 から 371、top entry pattern family count 合計は 333 から 314 へ改善した。`bach-001` と `fugue-smoke` は最頻 pattern count が 7 から 3 / 4 へ下がった。`minor-entry`、`modal-cadence`、`dense-modal` は改善するが、counter-subject identity retention、unison、shared rhythm、leap recovery の review-signal tradeoff が残るため、採用 baseline は譜面レビュー付きで扱う。22 seed の evidence は [Phase 11 review summary diagnostics](../reviews/phase-11-review-summary.md) に追記した。
9. 部分完了: unsupported thinning run に bass/root support を生成時に足す functional support formula を追加した。22 seed では unsupported functional-thinning run が 46 から 22、non-cadential low-density run が 346 から 317、strong beat bass/root support が 436 から 618 へ改善した。hard constraints と Phase 7B readiness は維持した。ただし harmonic function mismatch、unison、shared rhythm、leap recovery の review-signal tradeoff が残るため、support formula の採用は focused pairwise score review と合わせて扱う。22 seed の evidence は [Phase 11 review summary diagnostics](../reviews/phase-11-review-summary.md) に追記した。
10. 完了: focused seed set の before/after pairwise score review を記録した。`bach-001`、`fugue-smoke`、`minor-entry`、`modal-cadence`、`dense-modal` では Phase 10 score review の blocker である section grammar repetition と unsupported thinning が改善し、hard constraints、Phase 7B readiness、candidate-pool oracle shape、reference diagnostics summary は維持された。`minor-entry` の leap recovery、`modal-cadence` の counter-subject identity retention、`bach-001` / `fugue-smoke` の unison/shared rhythm、`dense-modal` の bass/root support は manual-listening follow-up として残すが、Phase 11 の採用 blocker ではなく継続 quality lane とする。pairwise score review は [Phase 11 review summary diagnostics](../reviews/phase-11-review-summary.md) に追記した。
11. 後回し: offline learned selection weight は、Phase 8/9 へ戻った後も selection-only upper bound が十分残る blocker だけで試す。採用判断ではなく、oracle-best と manual pairwise preference を説明できる feature を見つける診断 lane に留める。

## Relationship To Phase 8/9

Phase 8/9 は削除しない。ただし Phase 11 後の [Phase 11 post-completion score review](../reviews/phase-11-post-review.md) で、主題 family と subject-fragment の反復、modal/rotation seed の短い section cycle が残ることを確認した。無限再生、巻き戻し、操作パラメータ、Worker fallback は、この退屈さを長時間化して固定してしまうため、次は [Phase 12](phase-12.md) を先に実施する。
