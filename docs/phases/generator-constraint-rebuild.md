...
6. Downgrade or remove repair passes only after the corresponding solver constraint directly guarantees the behavior. Terminal-support candidate costs are now in place for terminal continuation windows. Harmonic-stasis rearticulation, harmonic-continuity support, and the focused score-level support cleanup group are downgraded with before / after trace evidence. Texture voice-crossing repair is now score-level solver-owned for generated score deltas, with standard-bundle no-op classification preserved when no crossing delta exists.
