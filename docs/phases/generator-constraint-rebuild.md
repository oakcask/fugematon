...
The constrained-profile feasibility slice makes key and subject planning consult the active `WritingProfile` before entries are accepted. Constrained profiles now choose profile-realizable key / subject / answer combinations, run profile-aware adjacent voice-order repair as solver-owned score-level evidence, and replace final pitch projection with a no-op invariant candidate. If profile projection would still change a note, diagnostics expose the remaining hard failure instead of silently mutating the score.
