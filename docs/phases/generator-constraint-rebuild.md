...
7. Make constrained `WritingProfile` pitch projection an invariant rather than the cleanup mechanism. Complete for key / subject feasibility and the reported `music-box-n20` voice-order regression: final projection now emits a no-op candidate when generation satisfied the profile and a hard-failure candidate when it would have needed to rewrite notes.
