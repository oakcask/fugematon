...
* 成功条件は「数分聴いても明らかな禁則だらけにならず、主題の再現、声部の独立、緊張と解決が聴感上分かること」とする。
...
* Phase 3 以降では、主題の音型と調性機能を見て真応答と調性応答を選ぶ。
  * 主題が tonic-dominant の輪郭を強く持つ場合は、調性応答を優先する。
  * 短い主題断片や episode 内の模倣では、完全な応答ではなく5度関係の断片再提示として扱う。
* diagnostics では、主題 entry の form、開始 tick、声部に加えて、想定した調性、スケール度数、実際の pitch class 差分を追えるようにする。
