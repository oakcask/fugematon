...
* Phase 13Y: planned after Phase 13X2; current implementation target。entry-boundary continuity を bass-specific な first-bass / post-exposition-bass windows から、entry voice、entry order、entry form、section state、already-entered voices を持つ一般モデルへ広げる。non-bass entry windows、alternate entry-order stress cases、prepared collective articulation、carried / suspended / resolving / delayed / staggered support を score-window evidence として出し、Phase 8/9 の前に `organ-default` と `strict-counterpoint` の focused listening note を記録する。
