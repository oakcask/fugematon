...
* Phase 13X2: planned。Phase 13X 後の follow-up review で、ユーザー報告の「バスの応唱で残りのパートが不自然に休止する」症状は entry tick ではなく、first bass answer 後の continuation / free-counterpoint tail thinning だった可能性が高いことを確認した。22 seed 中 5 seed で bass-only free-counterpoint tail、21 seed で one-or-zero outside-voice thinning が出る。Phase 13Y の前に focused bass-answer-tail diagnostic、continuation texture candidate repair、tail-thinning scoring、role-visible piano-roll / review output、focused `organ-default` / `strict-counterpoint` notes を実装する。
* Phase 13Y: planned after Phase 13X2。entry-boundary continuity を bass-specific な first-bass / post-exposition-bass windows から、entry voice、entry order、entry form、section state、already-entered voices を持つ一般モデルへ広げる。non-bass entry windows、alternate entry-order stress cases、prepared collective articulation、carried / suspended / resolving / delayed / staggered support を score-window evidence として出し、Phase 8/9 の前に `organ-default` と `strict-counterpoint` の focused listening note を記録する。
