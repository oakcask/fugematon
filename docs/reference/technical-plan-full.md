...
* Phase 5 は音楽的品質ゲートとして扱う。
  ...
  * Phase 8 の無限再生セッションは、Phase 7B の gate policy reset 後に hard constraint 上は開始できる状態になった。ただし音楽美を operational lane より優先するため、Phase 7B、Phase 10、Phase 11、Phase 12、Phase 12P、Phase 13、Phase 13Q、Phase 13R、Phase 13S、Phase 13T の adoption、follow-up repair、music-beauty-first rewrite、entry-sonority rewrite 後も、現行 default の再レビューで見つかった repeated entry formula、voice coupling、fragment transformation 不足、modal counter-subject weakness、metric false readiness を Phase 13U で先に扱う。Phase 6-7 の美しさ metric と manual listening は quality lane の evidence とする。
...
* Phase 13U: planned。Phase 13T 後の current beauty replan review で、reference aggregate が green に見えても score-window review では repeated entry formula、全 seed voice coupling、fragment transformation 不足、modal counter-subject weakness が残ることを確認した。Phase 8/9 の前に reusable entry formula detector、entry-support generator repair、voice-pair independence localization、fragment transformation repair、modal counter-subject preservation、metric truthfulness gate を実装する。既存モデル互換性、旧 guardrail margin、旧 expected values は、譜面上の美しさと矛盾する場合は採用条件にしない。
* Phase 8: Phase 13U の score-window beauty rewrite 後に、無限再生セッションを実装する。ring buffer replay、segment boundary、state-change / boundary / mode-change メタイベント、continuous fugue / endless program / regenerative cycle のモードを扱う。ユーザー向け操作は高水準 command に絞り、細かな生成パラメータのスライダ化は後続課題にする。Phase 6-7 の美しさ metric と manual listening がすべて pass になることは開始条件にしないが、Phase 13U の score-window evidence、focused listening、そこで見つかった音楽的問題の修正 evidence は開始条件にする。UI が音楽的な退屈さや Phase 12 後の unison / repeated-note defects を隠す設計にならないよう、Phase 13U の review evidence baseline を前提にする。無限再生セッションは hard constraints、determinism、schema compatibility、reference diagnostics summary、candidate-pool oracle shape を守り、performance profile metadata、review signal、quality vector comparison を消さずに segment 前後で比較できることを条件にする。
* Phase 9: Phase 13U と Phase 8 後に Dedicated Web Worker による生成探索の分離、生成期限、フォールバック候補、長時間 visualizer stability を実装する。Worker fallback は hard constraints を満たす候補を返し、reference diagnostics と review signal を維持する。内部状態可視化は生成、描画、再生が分離されても破綻しないことを条件にする。Worker fallback や deadline policy は、Phase 13U の音楽美 repair を代替しない。
