...
* Phase 8: current implementation target after harmonic-continuity follow-up。Metrical generation repair と Phase 14 14C4 後に、無限再生セッションを実装する operational lane。ring buffer replay、segment boundary、state-change / boundary / mode-change メタイベント、continuous fugue / endless program / regenerative cycle のモードを扱う。ユーザー向け操作は高水準 command に絞り、細かな生成パラメータのスライダ化は後続課題にする。Phase 6-7 の美しさ metric と manual listening がすべて pass になることは開始条件にしないが、Phase 13X の first-bass repair evidence、Phase 13X2 の bass-answer-tail texture evidence and role visibility、Phase 13Y の generalized entry-continuity evidence、Phase 13Z の long-run phrase-development evidence、Phase 14 の score-led beauty and harmonic-continuity evidence、Metrical generation repair の time-signature / meter-context evidence は開始条件にする。UI が音楽的な退屈さ、same-family phrase fatigue、line-coupling defects、entry boundary reset、bass-answer tail thinning、metric false acceptance、unclear harmonic progression、拍子 metadata と聴感上の拍節感の不一致を隠す設計にならないよう、Phase 14 と Metrical generation repair の review evidence baseline を前提にする。無限再生セッションは hard constraints、determinism、schema compatibility、reference diagnostics summary、candidate-pool oracle shape を守り、performance profile metadata、review signal、quality vector comparison、meter consistency comparison を消さずに segment 前後で比較できることを条件にする。
