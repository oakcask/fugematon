...
* 譜面を遡る操作は外部要件に含めない。将来 history / rewind scope を扱う場合は、まずメモリ上のリングバッファとして検討する。
* seed はユーザが記録・再入力できるようにする。
* 任意の履歴再生機能の再現性を扱う後続 scope では、ノートイベントだけでなく、状態遷移やパラメータ変更などのメタイベントも履歴に含める。
...
現在の MVP scope は 3 つの再生モード semantics と segment snapshot contract に限る。後続で操作 surface を追加する場合も、音楽的な退屈さを隠すためではなく、次の状態遷移を選ぶための高水準 command として設計する。

内部状態をビジュアライザの鑑賞要素にする作業は後続 scope で扱う。将来扱う場合は、状態遷移、主題 family、answer transform、fragment derivation、調性領域、cadence preparation、density arc、疲労度または novelty budget を、過度な数値 UI ではなく、色、透明度、軌跡、強調表示、境界演出で見せる。

状態変更や履歴記録の実装は後続 scope で扱う。将来扱う場合は、状態変更を即時に生成済みノートへ反映せず、次の状態遷移または segment 境界から有効にし、必要な変更を state-change、boundary、mode-change などのメタイベントとして履歴に記録する。

Segment 境界からの生成再開は、ring buffer replay ではなく snapshot で扱う。snapshot contract は各 segment の開始 tick に対応し、同じ `generatorVersion` のもとで次 segment の生成を同じ結果へ戻すために必要な内部状態を持つ。最低限、tick / timebase、mode、主題 family、answer transform、fragment derivation、調性領域、cadence preparation、density arc、novelty / fatigue budget、section planner state、未解決の voice / role continuity、PRNG internal state、bounded past event context を含める。bounded past event context は、generator が candidate generation、scoring、diagnostics のために参照する過去の ScoreEvent / role / metrical / harmonic context の window であり、再生履歴 UI や ring buffer replay とは別の内部状態として扱う。snapshot の永続化、圧縮、保持期間、UI 復元操作は後続 scope とする。

初回生成も同じ snapshot model で扱う。seed 文字列は generator へ直接渡す低水準入力ではなく、空の bounded past event context と seed 文字列から initial segment snapshot を作る高水準 command の入力である。initial segment snapshot には seed 由来の PRNG internal state、調性、拍子、テンポ、主題生成初期値、section planner の初期文脈、空 context を含める。Generator は seed 文字列ではなく、この snapshot から最初の segment を生成する。
...
* Phase 5 は音楽的品質ゲートとして扱う。
  ...
  * Phase 8 の無限再生セッションは、Phase 7B の gate policy reset 後に hard constraint 上は開始できる状態になった。ただし音楽美を operational lane より優先するため、Phase 7B、Phase 10、Phase 11、Phase 12、Phase 12P、Phase 13、Phase 13Q、Phase 13R、Phase 13S、Phase 13T、Phase 13U 後も、post-13U の譜面レビューで見つかった line agency、entry formula recurrence、counter-subject survivability、long-run development、metric false acceptance を Phase 13V で先に扱った。Phase 13V 後に確認した post-exposition bass entry 境界の外声同時再発音は Phase 13W で先に扱った。Phase 13W が除外していた exposition 初回 bass answer の外声同時再発音は Phase 13X で先に扱った。Phase 13X 後に、ユーザー報告の本命が entry tick ではなく first bass answer 後の free-counterpoint tail thinning だった可能性を確認したため、Phase 13X2 で corrected symptom と piano-roll role visibility を先に扱った。Phase 13Y では entry-boundary continuity を bass 固有ではなく entry voice、entry order、already-entered voices に基づく一般モデルへ広げた。Phase 13Z では long-run phrase convergence を subject-return、episode、stretto-like の発展不足として扱った。Phase 14 では、指標が譜面上の美しさを証明していない問題を score-window musical acceptance、entry continuity、line agency、counter-subject survivability、phrase development、metric truthfulness / CI scope cleanup の統合再設計として扱った。Metrical generation repair では、time-signature metadata と生成構造が一致しない問題を Phase 8/9 前に扱った。ユーザー報告の `seed-1dxb2n8-1miapx7` measures 6-7 は、short modulatory / pivot-harmony episode の harmonic-continuity failure として Phase 14 follow-up に戻した。最新レビューでは `fugue-smoke` measure 7 と関連 seed に short-episode harmonic-continuity failure が残り、`fugue-smoke` measure 5 と関連 seed に off-downbeat section-transition rhythm が残るため、Phase 14 を generalized harmonic-continuity and section-transition rhythm repair として再オープンした。Phase 8/9 の playback、visualizer、segment boundary はこの rhythm / harmony blocker を隠さない。ユーザー報告の `seed-0i335vx-1n54a1x` measures 5, 9, and 28 は、one-outside bass-answer tail、exposed free-counterpoint solo、退屈な exposed formula として Texture continuity repair に戻し、Phase 8/9 の segment boundary、playback smoothing、visualizer で隠さない。Historical reference calibration 後のユーザー確認では、自由唱が短いこと自体より、主題や対主題の断片・脚色として聞こえないことが残る blocker と判断したため、Episode motivic development を Phase 8/9 前に挿入する。Episode motivic development 後の `seed-07mwf08-1te3e2o` review では、motivic free-counterpoint でも同音連打と和声停滞へ落ちる問題を確認したため、Harmonic stasis rearticulation repair を Phase 8/9 前に挿入する。`seed-1db5j19-1nhjtae` review では、7-8小節のひどい和音が seed 固有ではなく、stretto / entry 周辺の unresolved accented clash と thin support の seed 横断問題として確認されたため、Stretto entry harmony repair を Phase 8/9 前に挿入し、completion review で修理済み evidence を記録した。Phase 6-7 の美しさ metric と manual listening は quality lane の evidence とする。
...
* 境界値 seed は、テンポ下限、テンポ上限、拍子、調性、entry spacing、density arc など、譜面上で観察できる初期文脈を狙う。
...
* state-change や mode-change は、標準 MIDI 再生には影響しないため、必要に応じてテキスト系メタイベントとして出力する。
* MIDI エクスポートは、音楽確認用であり、Fugematon 独自の全メタデータを完全保存する形式とは別に考える。
* Fugematon 独自の完全な再現データは、ScoreEvent 列、seed、selection model、segment snapshot を保存する形式で扱う。
...
* 保存する情報：
  ...
  * seed
    * 初回生成 command の入力文字列。
    * generator へ直接渡す低水準状態ではなく、空の bounded past event context と合わせて initial segment snapshot を作るために使う。
    * 調性、拍子、テンポ、主題、初期パラメータ、PRNG internal state などを決める起点になる。
  * initialSnapshotState
    * initial segment snapshot を作るときの内部状態の初期値。
    * seed 由来の PRNG internal state、調性、拍子、テンポ、主題生成初期値、section planner の初期文脈を含む。
  * timebase
    * tick 解像度、テンポ、拍子、開始 tick などの時間基準。
    * 生成、再生、描画、MIDI エクスポートの共通基準になる。
  * generatedUntilTick
    * events がどの tick まで生成済みかを示す。
    * 途中再生や任意の履歴再生時に、保存済み events の有効範囲を判断するために使う。
  * segmentSnapshots
    * segment 境界から次 segment の生成を再開するための checkpoint 列。
    * startTick 0 の initial segment snapshot もここに含める。
    * 各 snapshot は startTick、timebase、mode、主題 family、answer transform、fragment derivation、調性領域、cadence preparation、density arc、novelty / fatigue budget、section planner state、未解決の voice / role continuity、PRNG internal state、bounded past event context を含む。
    * bounded past event context には、generator が次 segment の candidate generation、scoring、diagnostics に必要とする範囲の過去 ScoreEvent と、voice、role、metrical intent、harmonic context、section function の要約を含める。範囲は generator の contract として明示し、seed から暗黙に復元しない。
    * snapshot は event replay の代替ではなく、保存済み events を持たない segment 境界から生成を始めるための再開点である。
  * events
...
* algorithm version は、同じ seed と initial snapshot state から生成される ScoreEvent 列が変わり得る変更で更新する。
  * PRNG
  * seed 分布
  * 主題生成
  * 候補生成
...
* 完全再現は、「同じ generatorVersion、同じ seed、同じ selection model、同じ initial segment snapshot derivation、同じ MetaEvent 列」で成立するものとする。
* Segment 境界からの局所再現は、「同じ generatorVersion、同じ segment snapshot、snapshot 以後の同じ MetaEvent 列」で成立するものとする。segment snapshot には PRNG internal state と bounded past event context を含め、seed や保存済み events だけから推測しない。
* 再生モードは以下を分けて考える。
  * event replay
    * 保存済み events をそのまま再生する。
    * generatorVersion が変わっても、保存済みイベント範囲内では同じ演奏になる。
  * deterministic regeneration
    * seed、selection model、MetaEvent 列から initial segment snapshot を再構築し、譜面を再生成する。
    * 同じ generatorVersion であれば同じ結果を期待できるが、保存済み範囲外の完全一致は保証しない。
  * segment snapshot generation
    * segment snapshot から次 segment の生成を始める。
    * 同じ generatorVersion と snapshot であれば、その segment 境界から継続生成した場合と同じ結果を期待できる。
* 譜面を遡る UI は必須ではない。将来扱う場合、保存済み範囲内では event replay を基本とする。
* 任意の履歴機能の MVP で過去地点からの再生を扱う場合は、event replay のみを正式機能にする。
* 過去地点から別分岐して続きを生成する branch mode は後続課題とする。
  * branch mode を導入する場合は、分岐元 tick、分岐元 replay id、分岐後の mode-change または boundary command を明示的に記録する。
  * 保存済み events を黙って書き換えず、別 branch として扱う。
* 保存済み範囲を超えて続きを生成する場合、対応する segment snapshot がないときは完全再現ではなく、同じ seed、selection model、MetaEvent 列を参考にして自然に続ける扱いとする。
* Fugematon が保証する完全一致は、保存済み events の範囲内、または同じ `generatorVersion` と segment snapshot から再生成する segment-local 範囲に限定する。
...
* Phase 5.9: Phase 5.6/5.7 の分解指標を review seed 全体へ広げ、counter-subject identity、rhythmic independence、unison、同方向進行、同一リズム、leap recovery、selected candidate の melody cost と texture cost に美しさ gate を置く。`fugue-smoke` では、アルト主題後の最初のソプラノ応唱が支え声部と不安定な 2 度衝突、応唱 local key の root と scale 外音の衝突、解決感の弱い 4 度構成を作らないことを回帰確認する。当時の manual listening blocker は current [agent score review policy](quality-metrics/agent-score-review-policy.md) により superseded とし、現在は高確信の score blocker だけを実装 blocker にする。
...
* Reference evaluation learning loop: Phase 10 の継続 quality lane を [current plan](../phases/reference-evaluation-learning-loop.md) で閉じる。実 reference score ingestion、generated/reference 共通 feature schema、blind A/B label validation、deterministic pairwise model、output-invariant shadow report、active-learning queue を実装し、manual listening 未実施と model adoption 未実施を implementation incompleteness から分離する。Default selection への昇格はこの plan の自動結果にせず、hard constraints、score-window review、holdout、reference-relative regression、local sentinel を含む別 adoption review を必要とする。
* Pairwise listening review tool: [sibling plan](../phases/pairwise-listening-review-tool.md) で、evaluation loop の bundle / response contract を使う local blind review application を実装する。A/B の performance context、renderer、shared playback position を統制し、回答前の model / diagnostics leakage を防ぎ、response を source bundle と別の versioned artifact へ atomic save する。Tool completion は preference winner や learned-model adoption を意味しない。
...
* Phase 13X2: complete。Phase 13X 後の follow-up review で、ユーザー報告の「バスの応唱で残りのパートが不自然に休止する」症状は entry tick ではなく、first bass answer 後の continuation / free-counterpoint tail thinning だった可能性が高いことを確認した。22 seed 中 5 seed で bass-only free-counterpoint tail、21 seed で one-or-zero outside-voice thinning が出ていた。Phase 13Y の前に focused `bassAnswerTailTexture` diagnostic、first-bass-answer tail support repair、role-visible piano-roll / review output、focused `organ-default` / `strict-counterpoint` notes を実装した。completion baseline は 22 seed の zero-outside / bass-only tail 0、first/post bass-entry synchronized reset 0 を保つ。残る one-outside tail span は Phase 13Y/13Z/14 で隠してはいけない review signal として残す。
* Phase 13Y: complete。entry-boundary continuity を bass-specific な first-bass / post-exposition-bass windows から、entry voice、entry order、entry form、section state、already-entered voices を持つ一般モデルへ広げた。22 seed review set で 666 important-entry windows と 478 non-bass windows を記録し、first-bass reset と bass-answer-tail reviewRequired は 0 を維持した。
* Phase 13Z: complete。Phase 13Y 後に、初期主題の多様性改善後も残る long-run phrase convergence を Phase 14/8/9 の前に扱った。`seed-0zereox-1v729ih` と focused seed sweep で確認した subject-return、episode、stretto-like の同一 subject-stem / fragment-family 反復を、`phase13ZReview` windowed diagnostics、guarded stem variation、history-aware novelty budgets、focused listening note で修正した。Phase 13X / Phase 13X2 / Phase 13Y の entry-continuity and tail-texture evidence、Phase 13V の line-agency / long-window evidence、hard constraints、Phase 7B readiness は維持されている。
* Phase 14: complete for Phase 8 handoff。Phase 13Z 後に、指標が譜面上の美しさを証明していない問題を Phase 8/9 の前に扱った。22 seed bundle と `random-listen-check` / `seed-0zereox-1v729ih` の ScoreEvent review で確認した reference aggregate false acceptance、1 声 carry だけでは不十分な entry-continuity classification、line agency の低さ、counter-subject survivability 不足、phrase development 不足を、score-window musical acceptance、generator candidate rebuild、metric truthfulness / scope cleanup、focused listening evidence で修正した。2026-05 refresh では、22 seed が Phase 7B ready のまま 288 local sentinel を出し、全 22 seed の first bass answer が 1 声 carry + 2 外声 reset でも `continuity-supported` になることを確認した。ユーザー報告後の ScoreEvent review では、古い bass-only first-answer collapse は再発しない一方、22 of 24 seeds で answer / stretto-like 後に 4-7 quarter の thin-support window が出て、free-counterpoint phrase signature も 15-20 seeds にまたがって収束することを確認した。Phase 13Z 後の focused inspection で、`contrary-motion` と `tight-stretto` に passing-tone / passing-neighbor semitone clash の局所悪化も確認したため、Phase 14 は 14A0 post-13Z dissonance triage、14A score-window harness、14B entry dissonance and continuity generation、14C0 post-entry support-continuity generation、14C1 free-counterpoint phrase-vocabulary generation、14C2 line-agency / counter-subject / phrase-development generation、14D metric truthfulness and scope cleanup、14E bundle/listening evidence の順で扱った。14C4 short-episode harmonic-continuity generation added `harmonicContinuity` score-window evidence for focused short modulatory or pivot-harmony episodes and bass-root / chord-tone support for the reported D minor to E minor pivot handoff. `seed-1dxb2n8-1miapx7` measures 6-7 are repaired as an `audible-progression` window. The later `fugue-smoke` review reopened the handoff for measure 7 `generator-response-required` short-episode harmonic continuity and measure 5 review-required exposition-to-episode transition rhythm. The reopened handoff is now complete: the first post-exposition continuation's shared weak-beat entry, phrase boundary, and harmonic anchor are classified consistently as `pickup-or-cross-metric`, and the generalized `harmonicContinuity` classification covers ascending-step, descending-step, circle-fifths, parallel-shift, sequence, contrary-motion, and inversion handoff windows. Two later same-mode `dense-modal` harmonic-continuity windows remain future review-required material rather than Phase 8 handoff blockers. Phase 13Z の phrase-development repair は rollback せず、phrase novelty や controlled ambiguity が unprepared semitone friction、unclear progression、または unsettled transition rhythm を増やす場合は future quality lane の generator / scoring response として扱う。既存モデル互換性、旧 guardrail margin、旧 expected values は、譜面上の美しさと矛盾する場合は採用条件にしない。
* Metrical generation repair: complete。`TimeSignature` 由来の meter context を generator、scoring、diagnostics へ接続し、3/4 focused seeds は exposition entry と subject accent を小節頭中心に修正した。6/8 focused seeds は downbeat だけでなく compound midpoint evidence を `meterConsistencyReview` に出す。`meterConsistencyReview` は `review-required` のまま、time-signature metadata と生成構造の不一致を説明する review evidence として Phase 8 に渡す。
* Texture continuity repair: complete。`seed-0i335vx-1n54a1x` の5小節目、9小節目、28小節目で確認した one-outside bass-answer tail、exposed free-counterpoint solo、退屈な exposed formula を、exposed-solo review surface、first-bass-answer tail texture floor、support generation、continuation texture-floor planning、exposed phrase grammar の順で扱った。`exposedFreeCounterpointSolo` は unsupported exposed free-counterpoint solo と cadence-preparation / pedal などの function-explained window を分ける review surface として Phase 8 に渡す。focused seed set は first bass-answer tail の zero-outside / bass-only / one-outside review-required window 0、unsupported exposed free-counterpoint solo 0 を完了 evidence とする。cadential thinning、pedal、echo、suspension preparation、deliberate solo rhetoric は残すが、score-window function で説明できる必要がある。
* Episode motivic development: complete。Historical reference calibration 後に、自由唱と episode が短いこと自体ではなく、主題・応答・対主題・cadence figure からの断片化、脚色、ゼクエンツ、反行、模倣、転調準備として説明できないことを Phase 8/9 前の品質 blocker として扱った。emitted note metadata と `episodeMotivicDevelopment` diagnostics は source motive、transformation kind、target function、episode derivation coverage、generic free-counterpoint duration、repeated episode formula review を出す。短い episode は clear function がある場合に残し、長い episode は measure count ではなく motivic and tonal function によって採用する。repeated stock formula は review-required follow-up として残す。
* Harmonic stasis rearticulation repair: complete。Episode motivic development 後に、主題・応答・対主題由来の rhythm / derivation がある自由唱でも、first bass answer 後、all-free texture、functional-support 後処理で同音連打と和声停滞へ落ちる問題を Phase 8/9 前の品質 blocker として扱った。`harmonicStasisRearticulation` diagnostics は same-voice short rearticulation、role、metrical intent、harmonic function、first-bass-answer handoff、all-free texture、motivic derivation を同じ score-window へ戻し、`scoreWindowAcceptance` でも見える。採用は diagnostic、post-processing support guard、candidate scoring、局所的な generator alternatives の順に行い、単純な長音化で line agency を失う修理は採用しない。
* Phase 8: complete implementation target after Harmonic stasis rearticulation repair and Stretto entry harmony repair。Metrical generation repair、Phase 14 14C4/14C5、Texture continuity repair、Historical reference calibration、Episode motivic development、Harmonic stasis rearticulation repair、Stretto entry harmony repair 後に、無限再生セッションの 3 再生モード semantics と segment snapshot contract を定義した operational lane。completed scope は `continuous fugue`、`endless program`、`regenerative cycle` の違いを、曲間、segment 終止、主題記憶、調性/密度/疲労度の引き継ぎ、section bridge、snapshot からの生成再開の観点で説明することに限る。Phase 6-7 の美しさ metric と manual listening がすべて pass になることは開始条件にしないが、Phase 13X の first-bass repair evidence、Phase 13X2 の bass-answer-tail texture evidence and role visibility、Phase 13Y の generalized entry-continuity evidence、Phase 13Z の long-run phrase-development evidence、Phase 14 の score-led beauty、generalized harmonic-continuity、section-transition rhythm evidence、Metrical generation repair の time-signature / meter-context evidence、Texture continuity repair の exposed-solo evidence、Historical reference calibration の voice-pair / entry-support evidence、Episode motivic development の derivation evidence、Harmonic stasis rearticulation repair の first-bass-answer handoff and functional-support provenance evidence、Stretto entry harmony repair の first-stretto / important-entry sonority evidence は開始条件にした。3 モード定義と snapshot contract は音楽的な退屈さ、same-family phrase fatigue、line-coupling defects、entry boundary reset、bass-answer tail thinning、exposed free-counterpoint filler、generic subject-free spans、motivic labels without harmonic tension / release、metric false acceptance、unclear harmonic progression、unsettled transition rhythm、stretto entry harmony failure、拍子 metadata と聴感上の拍節感の不一致を隠す設計にしない。
* Phase 9: Phase 8 後に Dedicated Web Worker による生成探索の分離、生成期限、フォールバック候補、長時間 visualizer stability を実装する。Worker fallback は hard constraints を満たす候補を返し、reference diagnostics と review signal を維持する。内部状態可視化は生成、描画、再生が分離されても破綻しないことを条件にする。Worker fallback や deadline policy は、Phase 13V / Phase 13W / Phase 13X / Phase 13X2 / Phase 13Y / Phase 13Z / Phase 14 の音楽美 repair や Metrical generation repair を代替しない。
...
* Phase 5 以降の CI で確認する項目：
  ...
  * deceptive motion、evaded cadence、pivot harmony、modulatory motion が、次の local key または harmonic anchor への到達として説明でき、短い episode では root support、chord-tone support、prepared dissonance、thinning rationale、section handoff が score window で聞き取れる。
...
* Phase 8 以降の CI または review harness で確認する項目：
  * 3 再生モード semantics は、既存の hard constraint、review signal、quality vector、meter consistency、texture-continuity evidence を隠す境界定義になっていない。
  * Seed 文字列と空の bounded past event context から initial segment snapshot を作り、その snapshot から初回 segment を生成できる。
  * Segment snapshot は PRNG internal state と bounded past event context を含み、同じ `generatorVersion` で snapshot から次 segment を生成した結果が、直前 segment 末尾から継続生成した結果と一致する。
* Phase 2 以降の手動またはブラウザテストで確認する項目：
  ...
  * 任意の履歴機能を実装する phase 以降は、指定地点から保存済み events を再生し直せる。
