...
* Phase 8: implementation target after Texture continuity repair。Metrical generation repair、Phase 14 14C4、Texture continuity repair 後に、無限再生セッションの 3 再生モード semantics と segment snapshot contract を定義する operational lane。current scope は `continuous fugue`、`endless program`、`regenerative cycle` の違いを、曲間、segment 終止、主題記憶、調性/密度/疲労度の引き継ぎ、section bridge、snapshot からの生成再開の観点で説明することに限る。Phase 6-7 の美しさ metric と manual listening がすべて pass になることは開始条件にしないが、Phase 13X の first-bass repair evidence、Phase 13X2 の bass-answer-tail texture evidence and role visibility、Phase 13Y の generalized entry-continuity evidence、Phase 13Z の long-run phrase-development evidence、Phase 14 の score-led beauty and harmonic-continuity evidence、Metrical generation repair の time-signature / meter-context evidence、Texture continuity repair の exposed-solo evidence は開始条件にする。3 モード定義と snapshot contract が音楽的な退屈さ、same-family phrase fatigue、line-coupling defects、entry boundary reset、bass-answer tail thinning、exposed free-counterpoint filler、metric false acceptance、unclear harmonic progression、拍子 metadata と聴感上の拍節感の不一致を隠す設計にならないよう、Phase 14、Metrical generation repair、Texture continuity repair の review evidence baseline を前提にする。
