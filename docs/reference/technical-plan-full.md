...
* Phase 5 は音楽的品質ゲートとして扱う。
  ...
  * Phase 8 の操作パラメータは、Phase 7B の gate policy reset 後に増やす。Phase 7B は完了しており、Phase 6-7 の美しさ metric と manual listening は quality lane の evidence とする。操作機能開始条件は hard constraints、determinism、schema compatibility、reference diagnostics summary、candidate-pool oracle shape の通過であり、全 seed の musical beauty pass ではない。
