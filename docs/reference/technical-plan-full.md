...
* Phase 5 は音楽的品質ゲートとして扱う。
  ...
  * Phase 8 の無限再生セッションは、Phase 7B の gate policy reset 後に hard constraint 上は開始できる状態になった。ただし音楽美を operational lane より優先するため、Phase 7B、Phase 10、Phase 11、Phase 12、Phase 12P、Phase 13、Phase 13Q、Phase 13R、Phase 13S の adoption、follow-up repair、music-beauty-first rewrite 後も、現行 default の再レビューで見つかった entry sonority、voice-pair lockstep、pitch-class unison、fragment function、modal counter-subject identity を Phase 13T で先に扱う。Phase 6-7 の美しさ metric と manual listening は quality lane の evidence とする。
...
* Phase 13T: planned。Phase 13S 後の current beauty audit で、entry window が pitch-class unison stack と隣接 2度/7度 friction を反復し、duration-based lockstep と pitch-class unison が全 22 seed で外れ、subject-fragment vocabulary と modal counter-subject identity がまだ弱いことを確認した。Phase 8/9 の前に entry-window sonority classifier、voice-pair independence repair、fragment-function evidence、modal counter-subject window review を実装し、score-window evidence と focused listening note で metrics の説明力を確認する。既存 axis が音楽的原因を隠す場合は、metric を split、rename、demote、remove、または role-aware local sentinel へ置き換え、generated score の改善と diagnostic reclassification を分けて記録する。
* Phase 8: Phase 13T の voice-independence and entry-sonority rewrite 後に、無限再生セッションを実装する。ring buffer replay、segment boundary、state-change / boundary / mode-change メタイベント、continuous fugue / endless program / regenerative cycle のモードを扱う。ユーザー向け操作は高水準 command に絞り、細かな生成パラメータのスライダ化は後続課題にする。Phase 6-7 の美しさ metric と manual listening がすべて pass になることは開始条件にしないが、Phase 13T の score-window evidence、focused listening、そこで見つかった音楽的問題の修正 evidence は開始条件にする。UI が音楽的な退屈さや Phase 12 後の unison / repeated-note defects を隠す設計にならないよう、Phase 13T の review evidence baseline を前提にする。無限再生セッションは hard constraints、determinism、schema compatibility、reference diagnostics summary、candidate-pool oracle shape を守り、performance profile metadata、review signal、quality vector comparison を消さずに segment 前後で比較できることを条件にする。
* Phase 9: Phase 13T と Phase 8 後に Dedicated Web Worker による生成探索の分離、生成期限、フォールバック候補、長時間 visualizer stability を実装する。Worker fallback は hard constraints を満たす候補を返し、reference diagnostics と review signal を維持する。内部状態可視化は生成、描画、再生が分離されても破綻しないことを条件にする。Worker fallback や deadline policy は、Phase 13T の音楽美 repair を代替しない。
