...
状態変更や履歴記録の実装は後続 scope で扱う。将来扱う場合は、状態変更を即時に生成済みノートへ反映せず、次の状態遷移または segment 境界から有効にし、必要な変更を state-change、boundary、mode-change、parameter-change などのメタイベントとして履歴に記録する。
...
* 保存する情報：
  ...
  * initialParameters
    * initial segment snapshot を作るときの生成パラメータと内部状態の初期値。
...
* 完全再現は、「同じ generatorVersion、同じ seed、同じ initialParameters、同じ initial segment snapshot derivation、同じ MetaEvent 列」で成立するものとする。
...
* 再生モードは以下を分けて考える。
  ...
  * deterministic regeneration
    * seed、initialParameters、MetaEvent 列から initial segment snapshot を再構築し、譜面を再生成する。
...
* 過去地点から別分岐して続きを生成する branch mode は後続課題とする。
  * branch mode を導入する場合は、分岐元 tick、分岐元 replay id、分岐後の parameter-change を明示的に記録する。
  * 保存済み events を黙って書き換えず、別 branch として扱う。
* 保存済み範囲を超えて続きを生成する場合、対応する segment snapshot がないときは完全再現ではなく、同じ seed、initialParameters、MetaEvent 列を参考にして自然に続ける扱いとする。
