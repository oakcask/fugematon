...
| `unresolved-entry-severe-interval` | entry 周辺の severe interval が full-beat 以上残る、または resolved / unresolved が混在して局所 review evidence として残る箇所。孤立した half-beat friction は duration axis に残し、local sentinel にはしない。 |
...
* `voicePairFunctions`: duration lockstep and pitch-class unison split into subject support, cadence support, sequence support, pedal-like support, mechanical coupling, exact collision, color doubling, and functional reinforcement. Subject support includes explicit planned-entry pickup provenance or a shared `prepare-subject-return` function; cadence and sequence support require cadence context or matching note-level function / transformation. A section label alone does not exempt outside-voice lockstep.
...
* `voicePairSpans`: localized exposed spans for exact collision, pitch-class reinforcement, color doubling, subject support, cadence support, sequence support, or mechanical coupling. The bounded review list retains up to two longest spans from every classification before filling remaining slots by duration, so long functional imitation cannot hide shorter review-required coupling or collision windows.
...
Continuation candidate selection uses the same functional split. Subject / answer support, cadence, sequence, and pedal-like synchronization remain accepted functional evidence; unexplained mechanical coupling, exact collision, and color doubling contribute separate soft costs. The broad historical lockstep and unison axes remain compatible diagnostics, but local selection must not improve them by worsening entry identity, harmonic support, leap recovery, or phrase development.
