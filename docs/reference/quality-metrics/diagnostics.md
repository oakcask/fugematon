...
| `constraintSatisfactionReview` | continuation section を voice/time slot CSP として読み、slot value を internal `note` / `hold` / `intentional-rest` として分類する summary。 | schema 1 は public `RestEvent` を追加せず、allowed rest reason、min/max active voices、intentional rest span、unplanned silent run、unsupported solo、all-voice silence、structural root/chord support miss、relaxation level、solver candidate count を出す。`section-constraint-csp-local-first` claim に基づく local-first solver evidence であり、CI hard gate 昇格は review evidence 後に分けて扱う。 |
