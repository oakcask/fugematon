...
| `terminalClosureReview` | endless-program / regenerative-cycle の segment 末尾が authentic または modal cadence、低声部 support、外声 landing、未解決 dissonance、texture thinning、final rest の観点で終止しているかを見る summary。 | `continuous-fugue` では `not-required`。`endless-program` では accepted / review-required / generator-response-required を UI と worker snapshot に残し、fallback、boundary silence、playback smoothing で弱い終止を隠さない。 |
