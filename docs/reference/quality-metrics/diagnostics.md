...
| `constraintSatisfactionReview` | continuation section を voice/time slot CSP として読み、slot value を internal `note` / `hold` / `intentional-rest` として分類する summary。 | schema 2 は public `RestEvent` を追加せず、allowed rest reason、min/max active voices、intentional rest span、unplanned silent run、unsupported solo、all-voice silence、structural root/chord support miss、relaxation level、solver candidate count に加えて、`metricalBoundaryCost`、off-measure phrase / harmonic-anchor / entry-start counts、unprepared transition count、prepared pickup count を出す。`section-constraint-csp-local-first` claim に基づく local-first solver evidence であり、CI hard gate 昇格は review evidence 後に分けて扱う。 |
...
| `constraintSatisfactionReview` | Section-local CSP summary。 | `generatorSearchTrace` の section rows と合わせ、rest 相当の silence が allowed internal reason を持つか、non-cadential continuation で long unplanned silent run や unsupported solo が残るか、structural anchors に root/chord support があるかを読む。 |
