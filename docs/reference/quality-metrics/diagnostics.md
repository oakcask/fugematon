...
| `constraintSatisfactionReview` | continuation section を voice/time slot CSP として読み、slot value を internal `note` / `hold` / `intentional-rest` として分類する summary。 | schema 5 は public `RestEvent` を追加せず、schema 4 の density / metrical-boundary / harmonic-quality / planned-entry / voice-pair evidence に加えて、`upperVoiceThematicMonopolyCount`、`lowerVoiceFillerDominanceCount`、`supportFillerLockstepCount`、`lowerLineContinuityGapCount`、`freeCounterpointScarcityCount`、`shortStructuralSupportChurnCount` を出す。複数声部の `register-relief` / `entry-handoff-delay` が同時に起き、cadence、pedal、active entry support なしに active voice floor を割る場合は unplanned silence / density failure として読む。`entryPlanViolationCount` と `nonChordStructuralSupportCount` は candidate rejection へ接続し、entry support / severe interval / unison / lockstep / sustained dissonance / voice agency は section-CSP soft cost として扱う。 |
...
### constraintSatisfactionReview schema 5 voice-agency counts
