...
| `unresolvedAccentedEntryClashCount` | accent 上の severe entry clash が resolution deadline までに説明されない score window。CLI review summary は `dissonanceTriage` の window count を使い、`constraintSatisfactionReview.infeasibleConstraintCounts` は section checkpoint count として別に保持する。 | count だけで hard failure にせず、accent、entry identity、support line の解決先を合わせて読む。window count と checkpoint count を混ぜて before / after 比較しない。 |
...
| `importantEntryTailTexture` | subject / answer / episode subject-fragment entry を、entry 声部と already-entered voices の半拍 window として読み、entry start から entry end + 4拍までの outside support 密度を見る summary。 | schema 1 は `zeroOutsideVoiceTicks`、`oneOutsideVoiceTicks`、`minOutsideVoiceCount`、`activeOutsideVoices`、`supported-tail` / `one-outside-tail` / `zero-outside-tail` classification を持つ。hard failure にはせず、`important-entry-tail-texture` soft cost と `entry-window` continuation candidate ranking で扱う。意図的な thinning はあり得るため、まず review-required evidence とする。 |
...
| `constraintSatisfactionReview` | continuation section を voice/time slot CSP として読み、slot value を internal `note` / `hold` / `intentional-rest` として分類する summary。 | schema 7 は public `RestEvent` を追加せず、density window と structural support に加えて各 section start / end の拍節境界を section state、planned entry、cadence preparation、pickup preparation で分類する。downbeat、prepared pickup、planned-entry pickup、cadence rhetoric は accepted context とし、未準備遷移だけを duration / candidate search の generator-response cost にする。density / structural hard rejection、harmonic quality、entry support、unison、lockstep、sustained dissonance、voice agency の policy は schema 6 から維持する。 |
| `scoreWindowAcceptance` | important entry、subject-fragment entry support、free-counterpoint solo、harmonic sonority、dissonance triage、active voice-pair span、counter-subject survival、phrase development、metric explanation を同じ window list へ束ねる score-window acceptance surface。Voice-pair span は bounded list 内で classification ごとの代表を保持する。 | Aggregate metrics より先に読む review surface。各 window の tick、state、voices、roles、classification、theory basis、response から、accepted context / review-required / generator response / diagnostic context を分ける。長い subject / cadence support だけで review-required coupling window が list 外へ落ちていないことも確認する。 |
...
| `bassAnswerTailTexture` | 初回 bass answer の後半から continuation / free-counterpoint tail が bass-only、zero-outside、one-outside-voice へ薄くなるか、追加 support rhythm が拍節・動機で説明できるかを見る focused compatibility summary。 | Phase 13X2 で追加。`importantEntryTailTexture` が汎用 entry-local texture surface になった後も、first-bass-answer 固有の rhythm coherence と既存 review/baseline 互換のため残す。schema 3 は answer 内部の5拍目以降も window に含め、付点 rhythm を禁止せず、subject / answer / counter-subject 由来でない off-grid dotted tail support を `unmotivated-tail-fragmentation` として review-required にする。 |
...
| `generatorSearchTrace` | deterministic local solver / diagnostics-only trace。 | Solver mode は exposition candidate に加えて、section-local continuation candidate rows を `section-<tick>-<state>-<band>-duration-<ticks>-candidate-<index>` の stable id で出す。各 row は hard failure code、soft-cost reason、relaxation、代表 affected note を持つ。`section-csp` band は continuation selection 前に評価された CSP-owned variants、`entry-window` band は important entry tail texture と planned-entry preparation、`voice-independence` band は mechanical pair rhythm の局所 stagger candidate を表す。Episode / free-counterpoint rows は motivic derivation、harmony realization、independent contour / rhythm、clash resolution、entry handoff support を説明する。Score-level support cleanup は stage-specific stable id の unrepaired / proposed rows で final repair replacement evidence を残し、通常採用経路には戻さない。 |
...
### constraintSatisfactionReview schema 7 classifications and voice-agency counts

`metricalBoundaryClassifications` は section start / end ごとに `plannedEntry`、`cadencePreparation`、`pickupPreparation`、classification、response、cost を記録する。`meter-confirming`、`compound-meter-continuation`、`prepared-pickup`、`planned-entry-pickup`、`cadence-rhetoric` は accepted context であり、off-measure であることだけでは罰しない。`unprepared-transition` だけが `generator-response-required` となる。Duration search は現在位置から小節頭へ着地する近傍候補も試すが、prepared pickup や cadence rhetoric を壊して全 section を小節頭へ強制しない。

各 section window の `densityClassifications` は、required floor を割る silence を `cadence-breath`、`entry-handoff-delay`、`pedal-thinning`、`suspension-resolution`、`register-relief`、または `unsupported-collapse` として記録する。`structuralSupportClassifications` は support-role の実 pitch を `chord-tone-support`、`passing-tone`、`neighbor-tone`、`suspension`、`unsupported-structural-label` として記録する。前者の unsupported collapse と後者の unsupported label だけが density / structural hard count に入る。

`upperVoiceThematicMonopolyCount` は non-exposition の non-cadential checkpoint で、ソプラノが subject / answer / subject-fragment / counter-subject だけを歌い、低中声部だけが free-counterpoint または structural support を担う場合の review count。`lowerVoiceFillerDominanceCount` は active voice floor を満たしていても、低中声部の filler / support に texture が偏り、ソプラノに free-counterpoint agency がない checkpoint を数える。`supportFillerLockstepCount` は structural-root / structural-chord-tone intent の free-counterpoint が他声部と同 start/duration で同期し、density support が filler 化して見える checkpoint を数える。

`lowerLineContinuityGapCount` は active voice floor を満たす non-cadential checkpoint でも、ソプラノを含む texture で alto / tenor / bass のどれかが欠け、低中声部の継続が薄くなる場合の soft evidence。`freeCounterpointScarcityCount` は planned entry / cadence 以外で subject / answer / subject-fragment / counter-subject だけが前面に出て、独立した free-counterpoint が不足する checkpoint を数える。`shortStructuralSupportChurnCount` は structural-root / structural-chord-tone support が短音価で同声部に隣接反復し、支えではなく断片的 filler として聞こえやすい箇所を数える。これらは hard failure ではなく、`section-csp-upper-line-agency` の voice-agency 配分 soft cost として扱う。

これらは `section-csp-upper-line-agency` soft cost にだけ接続する。Relaxation level、`sectionConstraintHardFailureCount`、CI hard gate には入れず、false acceptance を manual listening / review-required signal として残す。
...
| `importantEntryTailTexture` | important entry を entry-local multi-voice window として読み、already-entered voices の zero / one outside support thinning を汎用化した summary。first bass answer だけでなく non-bass subject / answer、post-exposition entry、stretto-like second entry、episode subject-fragment を同じ分類で比較する。 |
