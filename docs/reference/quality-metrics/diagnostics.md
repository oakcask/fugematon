...
| `constraintSatisfactionReview` | continuation section を voice/time slot CSP として読み、slot value を internal `note` / `hold` / `intentional-rest` として分類する summary。 | schema 6 は public `RestEvent` を追加せず、density window を function-explained / unsupported collapse、support-role pitch を chord support / passing / neighbor / suspension / unsupported structural label に分類する。`explainedDensityWindowCount`、`unsupportedDensityWindowCount`、`legitimateNonChordStructuralSupportCount`、`unsupportedStructuralLabelCount` が false positive と真の candidate rejection を分離する。required active-voice floor を維持する単独声部の休止は density failure にせず、`entryPlanViolationCount` と unsupported structural label だけを focused hard rejection へ接続する。metrical-boundary、harmonic quality、entry support、severe interval、unison、lockstep、sustained dissonance、voice agency は section-CSP soft cost として扱う。 |
...
| `generatorSearchTrace` | deterministic local solver / diagnostics-only trace。 | Solver mode は exposition candidate に加えて、section-local continuation candidate rows を `section-<tick>-<state>-<band>-duration-<ticks>-candidate-<index>` の stable id で出す。各 row は hard failure code と代表 affected note を持つ。`section-csp` band は continuation selection 前に評価された CSP-owned variants を表し、`entry-window` band は important entry tail texture を補う focused variants を表す。Episode / free-counterpoint rows は soft-cost reason で motivic derivation、harmony realization、independent contour / rhythm、clash resolution、entry handoff support を説明する。Score-level support cleanup は `score-*-support-*` の unrepaired / proposed rows で final repair replacement evidence を残す。 |
...
### constraintSatisfactionReview schema 6 classifications and voice-agency counts

各 section window の `densityClassifications` は、required floor を割る silence を `cadence-breath`、`entry-handoff-delay`、`pedal-thinning`、`suspension-resolution`、`register-relief`、または `unsupported-collapse` として記録する。`structuralSupportClassifications` は support-role の実 pitch を `chord-support`、`passing`、`neighbor`、`suspension`、`unsupported-structural-label` として記録する。前者の unsupported collapse と後者の unsupported label だけが density / structural hard count に入る。
