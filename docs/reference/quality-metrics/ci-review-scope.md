...
review profile 内の regression test は、1 test case あたり 1 seed を上限とし、同じ契約を確認できる最短の focused length を使います。複数 seed の傾向、rotation、A/B、過去 phase の quality baseline は自動 regression test に重複させず、`pnpm fugematon review` の標準 bundle と manual / A/B review で確認します。
...
* `review-required`: 1 seed の focused fugue-form continuation/carry checks、music-box focused contract、review CLI schema smoke、endless-program focused worker review signal。複数 seed の generator quality batch は review bundle に移し、test profile から削除します。

### Review test contraction

* touched seeds and metrics: fixed、boundary、rotation、adversarial seed を横断していた generator quality batch と、129600 ticks の worker review cases。
* classification: 重複する自動 batch は `remove-or-archive`、同じ seed 横断 evidence は review bundle 上の `review-required` または `manual-listening`。
* reason: 過去 phase の集計期待値を多数の test file で再生成しており、hard contract の修正先を示す focused sentinel と重複していた。
* action: 自動 review profile は 1 case / 1 seed と最短の成立長に限定し、複数 seed の傾向判断は標準 review bundle に残す。
* evidence gap: この変更は生成音楽の品質改善を主張しない。削除した seed 横断の美的比較は、必要な review ごとに bundle 生成と manual listening を行う。
