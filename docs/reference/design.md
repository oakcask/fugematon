...
* Phase 13S は、Phase 13R 後の 22 seed 譜面レビューにより挿入する。音楽的美しさをプロジェクトの核心として扱い、主題リズム、対主題、entry 対位法、声部独立、フーガ形式、指標の説明力を再設計する。既存モデル互換性、旧 guardrail margin、旧 expected values は、譜面上の美しさと矛盾する場合は採用条件にしない。現在は Phase 13S が Phase 8/9 前の実装対象である。
