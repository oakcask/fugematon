...
* Generation-quality implementation は人間の manual listening を待たず、[agent score review policy](quality-metrics/agent-score-review-policy.md) に従って完了判定する。AI が譜面から高確信で局在化した `score-blocking` failure は current metric に現れなくても修正し、音色や preference など譜面から判定できない残余は non-blocking `listening-gap` とする。
...
* Phase 8 は Phase 13X2 の bass-answer-tail texture repair、Phase 13Y の generalized entry-continuity repair、Phase 13Z の long-run phrase-development repair、Phase 14 の score-led musical beauty rebuild、Metrical generation repair、Texture continuity repair、Historical reference calibration、Episode motivic development、Harmonic stasis rearticulation repair、Stretto entry harmony repair 後に戻った operational lane として完了した。Phase 13V は line agency、entry formula novelty、counter-subject survivability、long-window development の generator-side evidence と focused listening note を記録した。Phase 13W は post-exposition bass entry 境界の外声同時再発音を generator-side blocker として修正した。Phase 13X は exposition 初回 bass answer の外声同時切断と再発音を generator-side blocker として修正した。Phase 13X2 は、ユーザー報告の本命と見られる first bass answer 後の free-counterpoint tail thinning と、主唱・応唱・対主題・自由唱を見分けにくい piano-roll observability を修正した。Phase 13Y は `entryBoundaryContinuity` を bass 固有ではなく、entry voice、entry order、already-entered voices に基づく review signal として一般化した。Phase 13Z は `phase13ZReview`、guarded stem variation、history-aware novelty budgets で subject-return、episode、stretto-like の same-family phrase fatigue を長時間再生前に扱った。Phase 14 は指標の pass ではなく譜面上の美しさを採用条件にし、entry continuity、line agency、counter-subject survivability、phrase development、metric truthfulness / CI scope cleanup を統合して扱った。Metrical generation repair は time-signature metadata を generator、scoring、diagnostics に接続し、3/4 や 6/8 の譜面が拍子構造としても聞こえることを Phase 8 の前提にした。Texture continuity repair と Historical reference calibration は、unsupported free-counterpoint solo、persistent voice-pair coupling、entry friction を Phase 8 の前提から外した。Episode motivic development は、自由唱と episode が主題・応答・対主題からの断片、脚色、ゼクエンツ、模倣、転調準備として説明できることを Phase 8 の前提にする。Harmonic stasis rearticulation repair は、motivic derivation がある自由唱でも pitch / harmony が緊張と安定を作れず、first bass answer 後、all-free texture、または functional-support 後処理で短い同音連打へ落ちる問題を Phase 8 の前提から外す。Stretto entry harmony repair は、stretto / entry 周辺の unresolved accented clash と thin support を Phase 8 の前提から外す。Phase 8 completed scope は 3 つの再生モードの musical / structural semantics と segment snapshot contract に限る。
* `continuous fugue`、`endless program`、`regenerative cycle` を同じ状態遷移 model の異なる境界表現として扱う。
* `continuous fugue` は明確な曲間を作らず、主題の再現、断片化、転回、移調、episode、stretto-like section を続ける。
* `endless program` は個々の曲または segment を意味的に終止させ、次の曲は前の主題 family、調性、密度、終止感、疲労度を受けて生成する。
* `regenerative cycle` は segment ごとに終止感を持たせながら、主題や状態の記憶を残して次の section へ橋渡しする。
* 初回生成の seed 文字列は generator へ直接渡す低水準値ではなく、空の bounded past event context と seed 文字列から initial segment snapshot を作る高水準 command の入力として扱う。generator はその snapshot の PRNG internal state と内部文脈から最初の segment を生成する。
* 各モードの segment 境界は、将来の replay storage とは別に、そこから次 segment の生成を始められる snapshot contract を持つ。snapshot は tick / timebase、mode、主題 family、answer transform、fragment derivation、調性領域、cadence preparation、density arc、novelty / fatigue budget、section planner state、未解決の voice / role continuity、PRNG internal state、generator が candidate generation / scoring / diagnostics に必要とする bounded past event context を含む。
* 成功条件は「3 モードの違いを、曲間、segment 終止、主題記憶、調性/密度/疲労度の引き継ぎ、section bridge、segment snapshot の観点で説明でき、既存の review signal を境界 semantics で隠さないこと」とする。
...
* Phase 13V は完了済みで、Phase 13W baseline になった。音楽的美しさをプロジェクトの核心として扱い、post-13U の譜面レビューで見つかった line agency、entry formula recurrence、counter-subject survivability、long-run development、metric false acceptance を Phase 13V で先に修正した。Phase 13X は exposition 初回 bass answer の外声同時再発音を修正した。Phase 13X2 は first bass answer 後の bass-only tail thinning と role visibility を修正した。Phase 13Y は entry-boundary continuity を entry voice、entry order、already-entered voices に基づく一般 review signal へ広げた。Phase 13Z は long-run phrase-development evidence を記録して完了した。Phase 14 は score-led beauty handoff として完了した。Metrical generation repair は拍子構造の handoff baseline として完了した。既存モデル互換性、旧 guardrail margin、旧 expected values は、譜面上の美しさや拍子構造と矛盾する場合は採用条件にしない。
...
* メタイベントは、任意の履歴再生と seed による再現性のために使う。

```ts
type ScoreEvent = NoteEvent | MetaEvent;

type MetaEvent = {
  type: "state-change" | "tempo-change";
  tick: number;
  payload: unknown;
};
```
...
* フーガ構造の明瞭さ:
  ...
  * 自由唱は generic filler ではなく、主題、応答、対主題、cadence figure、または既出 episode figure から導かれた断片化、脚色、ゼクエンツ、反行、模倣、 rhythmic paraphrase として説明できる。
...
## Seed 由来の initial snapshot

* seed 文字列は、空の bounded past event context から initial segment snapshot を作る高水準 command の入力として使う。generator は seed 文字列を直接参照せず、snapshot 内の PRNG internal state と初期文脈を参照する。
* seed 文字列は、明示的な疑似乱数生成器の初期化に使い、生成中に Math.random() は使わない。
* MVP では、seed から以下を決定する。
  * 調性
  * 拍子
  * テンポ
  * 主題生成の初期値
  * section planner の初期文脈
* 調性は、まず長調と短調のみを扱う。
  * 長調 55%、短調 45% を目安にする。
...
* 拍子は seed 由来の `TimeSignature` から meter context を作り、subject rhythm、entry spacing、harmonic anchors、cadence target、metrical intent、diagnostics に渡す。MVP の表示や export が 3/4 / 6/8 を示す場合、生成構造もその拍子を前提にする。
  * 4/4: 80%
  * 3/4: 15%
  * 6/8: 5%
...
* 操作用の数値パラメータは、生成判断に音楽的な意味を持つまでは public input にしない。
  * density や strictness のような抽象値は、譜面上の役割、section function、texture density arc、style profile へ分解してから導入する。
  * 将来ユーザ操作を追加する場合も、segment boundary から有効になる高水準 command として snapshot に反映し、未使用の数値を replay contract に残さない。
...
* episode では、主題断片や対主題断片を使いながら、次の主題再現へ向かう推移を生成する。自由唱は source motive、transformation kind、target function を持ち、単なる continuity filler と区別する。
