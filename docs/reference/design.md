...
* Phase 8 は Phase 13R の automatic default baseline / phrase convergence repair と required follow-up repair 後に戻る operational lane とする。Phase 6-7 の美しさ metric と聴取 gate の完全 pass は開始条件にしないが、Phase 13R の focused listening、seed 横断 subject-diversity evidence、そこで見つかった音楽的問題の修正 evidence は開始条件にする。細かな操作 UI が音楽的な退屈さを隠す設計にならないよう、generator quality、rendering boundary、model adoption evidence を前提にする。
...
* Phase 9 は Phase 13R required follow-up repair と Phase 8 後に戻る deferred operational lane とする。
