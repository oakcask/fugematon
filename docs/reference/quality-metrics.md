...
* [agent score review policy](quality-metrics/agent-score-review-policy.md): AI 譜面レビューによる実装完了、score blocker、non-blocking listening gap の扱い。
...
* agent score review verdict: metric scope とは別に、譜面所見を `score-blocking`、`score-concern`、`accepted` に分類する。Metric が存在しないことは、高確信で局在化された譜面上の破綻を実装 blocker にする妨げにならない。
* manual listening and pairwise preference: 自動指標では説明しきれない聴感上の判断。未実施であることを記録するが、implementation completion blocker にはしない。音色、再生、音響そのものが実装ターゲットである場合だけ、そのターゲットに必要な聴取または音響 evidence を別途定義する。
